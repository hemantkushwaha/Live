...
export function registerPrivateCallHandlers(io: SocketIOServer, socket: AuthenticatedSocket) {
  ...
  const handleRequestPrivateCall = ({ streamerId }: { streamerId: string }, ack?: (res: unknown) => void) => {
    try {
      ...
      io.to(streamer.socketId).emit(SOCKET_EVENTS.PRIVATE_REQUEST_RECEIVED, request);
  ...
  const handleRespondPrivateCall = (
    ...
    try {
      ...
      if (accept) {
        ...
        io.to(viewer.socketId).emit(SOCKET_EVENTS.PRIVATE_ACCEPTED, {
          session,
          streamerId: user.id,
          streamerName: user.username,
        });
        ...
        socket.emit(SOCKET_EVENTS.PRIVATE_STARTED, {
          session,
          peerId: request.viewerId,
          peerName: viewer.username,
          peerEmail: viewer.email,
        ...
        io.to(viewer.socketId).emit(SOCKET_EVENTS.PRIVATE_REJECTED, {
          requestId,
          reason: 'Streamer declined the private call request.',
        });
      }
  ...
  const handleEndPrivateCall = ({ sessionId }: { sessionId: string }, ack?: (res: unknown) => void) => {
    try {
      ...
      if (partner?.socketId) {
        ...
        io.to(partner.socketId).emit(SOCKET_EVENTS.PRIVATE_ENDED, {
          sessionId,
          endedBy: user.id,
        });
      }
      ...
      socket.emit(SOCKET_EVENTS.PRIVATE_ENDED, {
        sessionId,
        endedBy: user.id,
      });
  ...
  // Attach handlers
  socket.on(SOCKET_EVENTS.REQUEST_PRIVATE_CALL, handleRequestPrivateCall);
  socket.on(SOCKET_EVENTS.PRIVATE_REQUEST, handleRequestPrivateCall);

  socket.on(SOCKET_EVENTS.RESPOND_PRIVATE_CALL, handleRespondPrivateCall);
  socket.on(SOCKET_EVENTS.PRIVATE_ACCEPT, (data, ack) => handleRespondPrivateCall({ ...data, accept: true }, ack));
  socket.on(SOCKET_EVENTS.PRIVATE_REJECT, (data, ack) => handleRespondPrivateCall({ ...data, accept: false }, ack));

  socket.on(SOCKET_EVENTS.END_PRIVATE_CALL, handleEndPrivateCall);
  socket.on(SOCKET_EVENTS.PRIVATE_END, handleEndPrivateCall);
}
