...
export function registerStreamHandlers(io: SocketIOServer, socket: AuthenticatedSocket) {
  ...
  socket.on(SOCKET_EVENTS.START_STREAM, ({ title }: { title: string }, ack?: (res: unknown) => void) => {
    try {
  ...
  socket.on(SOCKET_EVENTS.STOP_STREAM, (ack?: (res: unknown) => void) => {
    try {
